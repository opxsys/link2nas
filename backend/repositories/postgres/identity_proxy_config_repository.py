...
class IdentityProxyConfigRepository:
        ...
        with self.db.connect() as conn:
            conn.execute(
                ...
                (
                    ...
                    config.enabled, config.label,
                    config.auto_login, config.auto_create_users,
                    config.allowed_domains_json, config.config_json,
    ...
    def get_by_id(self, config_id: str) -> IdentityProxyConfig | None:
        with self.db.connect() as conn:
            row = conn.execute(
                "SELECT * FROM identity_proxy_configs WHERE id = %s",
                (config_id,),
            ).fetchone()
    ...
    def update(self, config: IdentityProxyConfig) -> None:
        with self.db.connect() as conn:
            conn.execute(
                ...
                SET name = %s, provider_type = %s, enabled = %s, label = %s,
                    auto_login = %s, auto_create_users = %s,
                    allowed_domains_json = %s, config_json = %s,
                    updated_at = %s
                WHERE id = %s
                """,
                (
    ...
    def delete(self, config_id: str) -> None:
        with self.db.connect() as conn:
            conn.execute(
                "DELETE FROM identity_proxy_configs WHERE id = %s",
                (config_id,),
            )
