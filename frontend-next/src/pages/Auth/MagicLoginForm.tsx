...
export default function MagicLoginForm({ smtpAvailable, onSetView }: Props) {
  ...
  if (sent) {
    return (
      <div className="py-2">
        ...
        <p className="mb-6 text-sm text-muted-foreground">{t('magicEmailSent')}</p>
        <button
          type="button"
          onClick={() => onSetView('login')}
  ...
  return (
    <form onSubmit={handleSubmit} noValidate>
      ...
      <h2 className="mb-6 text-lg font-semibold text-foreground">{t('magicLogin')}</h2>

      <div className="mb-5">
        <label htmlFor="magic-email" className={LABEL}>{t('email')}</label>
        <input
          id="magic-email"
          type="email"
          autoComplete="email"
