...
export default function JobsToolbar({ filters, providers, destinations, onFilter, onRefresh }: JobsToolbarProps) {
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <select value={filters.status} onChange={(e) => onFilter('status', e.target.value)} className={SELECT} aria-label="Filter by status">
        <option value="">All Statuses</option>
        {JOB_STATUS_OPTIONS.map(({ value, label }) => <option key={value} value={value}>{label}</option>)}
      </select>

      <select value={filters.provider} onChange={(e) => onFilter('provider', e.target.value)} className={SELECT} aria-label="Filter by provider">
        <option value="">All Providers</option>
        {providers.map((p) => <option key={p} value={p}>{p}</option>)}
      </select>

      <select value={filters.destination} onChange={(e) => onFilter('destination', e.target.value)} className={SELECT} aria-label="Filter by destination">
        <option value="">All Destinations</option>
        {destinations.map((d) => <option key={d} value={d}>{d}</option>)}
      </select>

      <Button variant="outline" size="icon" onClick={onRefresh} aria-label="Refresh jobs list">
