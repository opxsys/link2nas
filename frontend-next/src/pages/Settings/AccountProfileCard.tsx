...
export default function AccountProfileCard({ me, onUpdate }: Props) {
  ...
  const LANGUAGES = [
    { value: '', label: t('langSystemDefault') },
  ...
  async function handleRequestVerification() {
    ...
    try {
      ...
      setVerifyMsg({ ok: res.ok, text: res.message ?? t('verificationEmailSent') })
    } catch (err) {
      setVerifyMsg({ ok: false, text: err instanceof ApiError ? err.message : t('saveFailed') })
    } finally {
  ...
  const roleLabel = me.role === 'super_admin' ? t('roleSuperAdmin') : me.role === 'user' ? t('roleUser') : me.role
  ...
  return (
    <SectionCard title={t('sectionProfile')}>
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2">
            ...
            {me.email_verified ? (
              <span className="inline-flex items-center gap-1 rounded-full border border-green-200 bg-green-50 px-2 py-0.5 text-xs text-green-700 dark:border-green-800 dark:bg-green-950 dark:text-green-400">
                <CheckCircle2 size={10} aria-hidden="true" /> {t('verified')}
              </span>
            ) : (
              <span className="inline-flex items-center gap-1 rounded-full border border-amber-200 bg-amber-50 px-2 py-0.5 text-xs text-amber-700 dark:border-amber-800 dark:bg-amber-950 dark:text-amber-400">
                <AlertCircle size={10} aria-hidden="true" /> {t('emailNotVerified')}
              </span>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          <label htmlFor="acc-display-name" className={LABEL}>{t('labelDisplayName')}</label>
        ...
        <div>
          <label htmlFor="acc-email" className={LABEL}>{t('email')}</label>
        ...
        <div>
          <label htmlFor="acc-lang" className={LABEL}>{t('labelPreferredLang')}</label>
        ...
        <p className="text-xs text-muted-foreground sm:col-span-2">
          {t('roleAdminNote')}
          {me.last_login_at && (
            <> · {t('lastLogin')} {new Date(me.last_login_at).toLocaleString()}</>
          )}
        </p>
      </div>
      ...
      {verifyMsg && (
        <div className={`mt-3 flex items-center justify-between gap-2 rounded-md border px-3 py-2 text-xs ${
          ...
          <button onClick={() => setVerifyMsg(null)} className="shrink-0 opacity-60 hover:opacity-100" aria-label={t('dismiss')}>
      ...
      <div className="mt-4 flex flex-col gap-3 border-t border-border pt-4">
        ...
        {saved && (
          <div className="flex items-center justify-between gap-2 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-700 dark:border-emerald-800 dark:bg-emerald-950 dark:text-emerald-400">
            <span className="flex items-center gap-2"><CheckCircle2 size={14} aria-hidden="true" /> {t('profileSaved')}</span>
            <button onClick={() => setSaved(false)} className="shrink-0 opacity-60 hover:opacity-100" aria-label={t('dismiss')}>
        ...
        {error && (
          <div className="flex items-center justify-between gap-2 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700 dark:border-red-800 dark:bg-red-950 dark:text-red-400">
            <span>{error}</span>
            <button onClick={() => setError(null)} className="shrink-0 opacity-60 hover:opacity-100" aria-label={t('dismiss')}>
              <X size={14} aria-hidden="true" />
            </button>
          </div>
        )}
