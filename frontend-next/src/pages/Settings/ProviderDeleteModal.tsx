...
import { deleteProviderConfig } from '@/api/provider-configs'
...
export default function ProviderDeleteModal({ config, onClose, onDeleted }: Props) {
  const { t } = useI18n()
  ...
  async function handleDelete() {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : t('providerDeleteFailed'))
    } finally {
      setDeleting(false)
    }
  }
  ...
  return (
    <div
      ...
      role="dialog" aria-modal="true" aria-label={t('deleteProvider')}
      onMouseDown={(e) => { if (e.target === e.currentTarget) onClose() }}
    >
      <div className="w-full max-w-sm rounded-lg border border-border bg-card shadow-lg">
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          <h2 className="text-sm font-semibold text-foreground">{t('deleteProvider')}</h2>
