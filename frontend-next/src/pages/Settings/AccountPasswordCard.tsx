...
export default function AccountPasswordCard({ singleUserMode }: Props) {
  ...
  async function handleSave() {
    setError(null)
    if (!currentPw) { setError(t('currentPasswordRequired')); return }
    if (!newPw)     { setError(t('newPasswordRequired')); return }
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : t('passwordChangeFailed'))
    } finally {
      setSaving(false)
    }
  }
          ...
          <div className="mt-4 flex flex-col gap-3 border-t border-border pt-4">
            ...
            {saved && (
              <div className="flex items-center justify-between gap-2 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-700 dark:border-emerald-800 dark:bg-emerald-950 dark:text-emerald-400">
                <span className="flex items-center gap-2"><CheckCircle2 size={14} aria-hidden="true" /> {t('passwordChanged')}</span>
